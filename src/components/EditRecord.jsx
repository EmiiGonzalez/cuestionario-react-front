...
export const EditRecord = ({ url }) => {
    ...
    if (noHayError) {
      ...
      if (conError.length > 0) {
        conError.forEach((field) => {
          alertify.error(
            `No se puede actualizar el registro debido a error de dato repetido en el campo ${field}`
          );
        });
      }
      if (noValidos.length > 0) {
        noValidos.forEach((field) => {
          alertify.error(
            `No se puede actualizar el registro debido a un dato invalido en el campo ${field} `
          );
        });
      }
    }
  ...
  return (
    ...
    >
      ...
      <button className="btn btn-primary w-25 mx-2" type="submit">
        Enviar
      </button>
      <button className="btn btn-secondary w-25 mx-2" onClick={() => navigate("/")}>
        Volver
      </button>
      </div>
    </form>
